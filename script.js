document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function showFieldError(field, message) {
      ...
      if (wrapper) {
        ...
        if (!wrapper.querySelector('.' + ERROR_MESSAGE_CLASS)) {
    ...
    function validateField(field) {
      ...
      if (type === 'tel' && !validators.phone(value)) {
        showFieldError(field, getErrorMessage(field, 'phone'));
        return false;
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (isFormValid) {
        ...
        setTimeout(function () {
          ...
          contactForm.reset();
          fields.forEach(function (field) { clearFieldError(field); });
    ...
    contactForm.addEventListener('input', function (e) {
      const field = e.target;
      if (field.matches('input, textarea, select') &&
          field.classList.contains(FIELD_ERROR_CLASS)) {
        validateField(field);
      }
    });
  }
  ...
  const scrollElements = document.querySelectorAll(
    ...
    '.section-title, .section-subtitle, .card, .feature, ' +
    '.hero-content, .about-content, .service-item, .team-member, ' +
    '.testimonial, .pricing-card, .cta-content, .gallery-item'
  );
    ...
    const intersectionCallback = function (entries, observer) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          const siblings = el.parentElement ? Array.from(el.parentElement.children).filter(function (child) {
            return child.classList.contains(OBSERVE_CLASS) && !child.classList.contains(ANIMATION_CLASS);
          }) : [];

          const index = siblings.indexOf(el);
          const delay = index > 0 ? index * STAGGER_DELAY : 0;
      ...
      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          if (node.nodeType === 1) {
            ...
            node.querySelectorAll && node.querySelectorAll('.' + OBSERVE_CLASS).forEach(function (el) {
              scrollObserver.observe(el);
            });
          }
        });
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'b777b60f-eb42-46af-a4d7-3e43bd626c71',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'b777b60f-eb42-46af-a4d7-3e43bd626c71';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'b777b60f-eb42-46af-a4d7-3e43bd626c71';
